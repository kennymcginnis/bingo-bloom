...
export function createGame(title: string, size: number, words: string[]) {
	...
	const addGame = sqlite.prepare('INSERT INTO games (code, title, size, admin_token) VALUES (?, ?, ?, ?) RETURNING id')
...
export function joinGame(game: GameRow, name: string) {
	...
	sqlite
		.prepare(
			'INSERT INTO players (game_id, name, token, card_json, card_signature, rejoin_code) VALUES (?, ?, ?, ?, ?, ?)',
		)
...
export function gameState(game: GameRow, token: string) {
	...
	if (isAdmin) {
		const players = sqlite
			.prepare(
				'SELECT id, name, card_json, joined_at, claimed_at, verified FROM players WHERE game_id = ? ORDER BY joined_at',
			)
			.all(game.id) as Array<Pick<PlayerRow, 'id' | 'name' | 'card_json' | 'joined_at' | 'claimed_at' | 'verified'>>
		const markRows = sqlite
			.prepare(
				'SELECT marks.player_id, marks.cell_position FROM marks JOIN players ON players.id = marks.player_id WHERE players.game_id = ?',
			)
...
export function playerBoardState(game: GameRow, playerId: number) {
	const player = sqlite.prepare('SELECT * FROM players WHERE id = ? AND game_id = ?').get(playerId, game.id) as
		| PlayerRow
		| undefined
	if (!player) return null

	const words = sqlite.prepare('SELECT id, text, called FROM words WHERE game_id = ?').all(game.id) as Array<{
		id: number
		text: string
		called: number
	}>
...
export function addChatMessage(game: GameRow, token: string, text: string) {
	...
	const result = sqlite
		.prepare(
			'INSERT INTO chat_messages (game_id, player_id, sender_role, text) VALUES (?, ?, ?, ?) RETURNING id, created_at',
		)
		.get(game.id, player?.id ?? null, role, text) as { id: number; created_at: string }
	const message = {
		...result,
		playerId: player?.id ?? null,
		name: isHost ? 'Host' : player?.name ?? 'Player',
		role,
		text,
	}
...
export function setMark(game: GameRow, token: string, position: number, marked: boolean) {
	...
	if (marked)
		sqlite.prepare('INSERT OR IGNORE INTO marks (player_id, cell_position) VALUES (?, ?)').run(player.id, position)
...
export function endGame(game: GameRow) {
	sqlite
		.prepare('UPDATE games SET ended_at = COALESCE(ended_at, CURRENT_TIMESTAMP), joining_locked = 1 WHERE id = ?')
		.run(game.id)
	publishGameEvent(game.id, { type: 'state' })
}

export function cloneGame(game: GameRow) {
	const words = sqlite.prepare('SELECT text FROM words WHERE game_id = ? ORDER BY position').all(game.id) as Array<{
		text: string
	}>
	return createGame(
		game.title,
