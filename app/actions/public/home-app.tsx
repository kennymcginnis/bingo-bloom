...
const examples = [
  'Let’s circle back', 'You’re on mute', 'Quick question', 'Low-hanging fruit',
  'Can everyone see my screen?', 'Take this offline', 'Bandwidth', 'Hard stop',
  'Action item', 'Move the needle', 'Touch base', 'Deep dive', 'Parking lot',
  'Win-win', 'At the end of the day', 'Synergy', 'Double-click on that',
  'Best practice', 'Deliverable', 'Heads-down', 'Core competency', 'Ping me',
  'Take a step back', 'Who owns this?', 'Next steps',
].join('\n')
...
export const HomeApp = clientEntry(import.meta.url, function HomeApp(handle: Handle<HomeAppProps>) {
  ...
  const submitCreate = async (form: HTMLFormElement, signal: AbortSignal) => {
    ...
    const response = await fetch('/api/games', {
      ...
      body: JSON.stringify({ title: formData.get('title'), size: Number(formData.get('size')), words: formData.get('words') }), signal,
    })
    const result = (await response.json().catch(() => ({ error: 'The server returned an unexpected response.' }))) as { code?: string; adminToken?: string; error?: string }
    if (signal.aborted) return
    if (!response.ok || !result.code || !result.adminToken) {
      busy = false; error = result.error ?? 'Could not create the game.'; handle.update(); return
    }
    localStorage.removeItem(UI_STATE_KEY)
  ...
  return () => (
    <main className="home-shell">
      <header className="site-header">
        <a className="brand" href="/" aria-label="Bingo Bloom home"><span className="brand-mark" aria-hidden="true">B</span><span>Bingo Bloom</span></a>
        <button className="quiet-button" type="button" mix={on('click', () => changePanel('join'))}>Join a game <span aria-hidden="true">→</span></button>
      </header>
      <section className="hero">
        <div className="hero-copy">
          <p className="eyebrow"><span /> Team bingo, minus the paper cuts</p>
          <h1>Turn any list into a little friendly chaos.</h1>
          <p className="hero-lede">Create unique cards, invite the whole team, and let the server settle every “Bingo!”—fair and square.</p>
          <div className="proof-row"><span>✓ No accounts</span><span>✓ Phone ready</span><span>✓ Verified wins</span></div>
        </div>
        <div className="setup-card">
          <div className="tabs" role="tablist" aria-label="Get started">
            <button className={panel === 'create' ? 'active' : ''} role="tab" aria-selected={panel === 'create'} mix={on('click', () => changePanel('create'))}>Create</button>
            <button className={panel === 'join' ? 'active' : ''} role="tab" aria-selected={panel === 'join'} mix={on('click', () => changePanel('join'))}>Join</button>
          </div>
          {panel === 'create' ? (
            <form className="form-stack" mix={on('submit', (event, signal) => { event.preventDefault(); void submitCreate(event.currentTarget, signal) })}>
              <div><p className="step-label">01 / Name it</p><label htmlFor="title">Game title</label><input id="title" name="title" placeholder="Friday all-hands bingo" maxLength={80} value={draftTitle} required mix={[ref((element) => { titleElement = element }), on('input', (event) => { draftTitle = event.currentTarget.value; saveUiState(); handle.update() })]} /></div>
              <div>
                <p className="step-label">02 / Plan it</p>
                <div className="planning-fields">
                  <div><label htmlFor="size">Card size</label><select id="size" name="size" mix={on('change', (event) => { boardSize = Number(event.currentTarget.value); saveUiState(); handle.update() })}><option value="3" selected={boardSize === 3}>3 × 3 · quick</option><option value="4" selected={boardSize === 4}>4 × 4 · medium</option><option value="5" selected={boardSize === 5}>5 × 5 · classic</option><option value="6" selected={boardSize === 6}>6 × 6 · long game</option></select></div>
                  <div><label htmlFor="playerRange">Player count</label><select id="playerRange" name="playerRange" mix={on('change', (event) => { playerRange = event.currentTarget.value as PlayerRange; saveUiState(); handle.update() })}><option value="small" selected={playerRange === 'small'}>2–5 players</option><option value="medium" selected={playerRange === 'medium'}>6–15 players</option><option value="large" selected={playerRange === 'large'}>16+ players</option></select></div>
                </div>
                <Recommendation wordCount={wordCount} size={boardSize} playerRange={playerRange} />
              </div>
              <div><p className="step-label">03 / Fill it</p><div className="phrase-label-row"><label htmlFor="words">One phrase per line</label><div className="phrase-actions"><button className="sort-button" type="button" disabled={duplicateEntries.length === 0} mix={on('click', () => { if (!wordsElement) return; draftWords = removeDuplicateEntries(wordsElement.value).join('\n'); wordsElement.value = draftWords; wordCount = uniqueCount(draftWords); duplicateEntries = []; error = ''; saveUiState(); handle.update(); wordsElement.focus() })}><span aria-hidden="true">−</span> Remove dups</button><button className="sort-button" type="button" mix={on('click', () => { if (!wordsElement) return; draftWords = splitEntries(wordsElement.value).sort((left, right) => left.localeCompare(right, undefined, { sensitivity: 'base' })).join('\n'); wordsElement.value = draftWords; wordCount = uniqueCount(draftWords); duplicateEntries = findDuplicateEntries(draftWords); saveUiState(); handle.update(); wordsElement.focus() })}><span aria-hidden="true">↕</span> Sort A–Z</button></div></div><textarea id="words" name="words" rows={8} defaultValue={draftWords} required aria-invalid={duplicateEntries.length > 0} aria-describedby={duplicateEntries.length ? 'duplicate-warning' : undefined} mix={[ref((element) => { wordsElement = element }), on('input', (event) => { draftWords = event.currentTarget.value; wordCount = uniqueCount(draftWords); duplicateEntries = findDuplicateEntries(draftWords); error = ''; saveUiState(); handle.update() })]} />{duplicateEntries.length > 0 && <p id="duplicate-warning" className="duplicate-warning" role="alert"><strong>Duplicate{duplicateEntries.length === 1 ? '' : 's'} found:</strong> {duplicateEntries.join(', ')}</p>}</div>
              {error && <p className="form-error" role="alert">{error}</p>}
              <button className="primary-button" type="submit" disabled={busy}>{busy ? 'Planting your game…' : 'Create my game'} <span aria-hidden="true">↗</span></button>
            </form>
          ) : (
      ...
      <section className="how-it-works" aria-label="How it works">
        ...
        <div className="steps"><article><b>1</b><h3>Paste your list</h3><p>Add phrases, moments, names—anything your team can spot or hear.</p></article><article><b>2</b><h3>Share one code</h3><p>Every teammate gets a different, mobile-friendly card instantly.</p></article><article><b>3</b><h3>Call it fair</h3><p>You mark official phrases. We check every claimed line against them.</p></article></div>
      </section>
    </main>
  )
})
...
function splitEntries(value: string) {
  return value.split(/\r?\n|,/).map((word) => word.trim().replace(/\s+/g, ' ')).filter(Boolean)
}
...
function Recommendation(handle: Handle<{ wordCount: number; size: number; playerRange: PlayerRange }>) {
  return () => {
    ...
    const status = wordCount < minimum ? `Add ${minimum - wordCount}–${maximum - wordCount} more` : wordCount > maximum ? 'Plenty of variety' : 'Sweet spot reached'
    return <div className={`recommendation-card ${wordCount >= minimum ? 'ready' : ''}`}>
      <div><span>Your list</span><strong>{wordCount}</strong><small>unique phrases</small></div>
      <span className="recommendation-arrow" aria-hidden="true">→</span>
      <div><span>Recommended</span><strong>{minimum}–{maximum}</strong><small>for {playerRange === 'small' ? '2–5' : playerRange === 'medium' ? '6–15' : '16+'} players</small></div>
      <b>{status}</b>
    </div>
  }
}
