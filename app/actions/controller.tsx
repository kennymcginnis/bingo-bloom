...
export default createController(routes, {
	actions: {
		...
		health() {
			try {
				return databaseHealth() ? json({ status: 'ok' }) : json({ status: 'unhealthy' }, 503)
			} catch {
		...
		async createGame({ request }) {
			...
			if (duplicates.length) {
				...
				return json(
					{ error: `Remove duplicate phrase${duplicates.length === 1 ? '' : 's'}: ${shown}${remaining}.` },
					400,
				)
			}
			if (words.length < requiredWords(size))
				return json({ error: `${size}×${size} cards need at least ${requiredWords(size)} unique phrases.` }, 400)
		...
		async joinGame({ request, params }) {
			...
			if (!rate.allowed)
				return json({ error: 'Too many join attempts. Try again shortly.' }, 429, {
		...
		async rejoinGame({ request, params }) {
			...
			if (!rate.allowed)
				return json({ error: 'Too many rejoin attempts. Try again shortly.' }, 429, {
					'Retry-After': String(rate.retryAfter),
				})
			const body = await readJson(request)
			const rejoinCode = String(body.rejoinCode ?? '')
				.trim()
				.toUpperCase()
		...
		async sendChatMessage({ request, params }) {
			...
			if (!rate.allowed)
				return json({ error: 'Chat is moving too quickly. Try again shortly.' }, 429, {
					'Retry-After': String(rate.retryAfter),
				})
			const body = await readJson(request)
		...
		async callWord({ request, params }) {
			...
			if (game.ended_at) return endedResponse()
			const body = await readJson(request)
			return setCalled(game, Number(body.wordId), Boolean(body.called))
				? json({ ok: true })
				: json({ error: 'Phrase not found.' }, 404)
		},
		...
		async lockGame({ request, params }) {
			...
			return setJoiningLocked(authorized.game, Boolean(body.locked)) ? json({ ok: true }) : endedResponse()
		},
