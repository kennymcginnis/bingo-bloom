...
it('runs a complete multiplayer game through the HTTP contract', async () => {
	...
	try {
		...
		const create = await router.fetch(
			request(routes.createGame.href(), {
				method: 'POST',
		...
		const joinResponse = await router.fetch(
			request(routes.joinGame.href({ code: created.code }), {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
		...
		const events = await router.fetch(
			request(routes.gameEvents.href({ code: created.code }), {
		...
		const chat = await router.fetch(
			request(routes.sendChatMessage.href({ code: created.code }), {
				method: 'POST',
		...
		const playerStateResponse = await router.fetch(
			request(routes.gameState.href({ code: created.code }), {
				headers: { Authorization: `Bearer ${joined.playerToken}` },
			}),
		)
		const playerState = await body<{ card: Array<{ id?: number; free?: boolean }>; marks: number[] }>(
			playerStateResponse,
		)
		...
		for (const wordId of winningWordIds) {
			const called = await router.fetch(
				request(routes.callWord.href({ code: created.code }), {
					method: 'POST',
		...
		for (const position of winningPositions) {
			const marked = await router.fetch(
				request(routes.markSquare.href({ code: created.code }), {
					method: 'POST',
					headers: { Authorization: `Bearer ${joined.playerToken}`, 'Content-Type': 'application/json' },
		...
		const claim = await router.fetch(
			request(routes.claimBingo.href({ code: created.code }), {
				method: 'POST',
				headers: { Authorization: `Bearer ${joined.playerToken}`, 'Content-Type': 'application/json' },
				body: '{}',
			}),
		...
		const lock = await router.fetch(
			request(routes.lockGame.href({ code: created.code }), {
				method: 'POST',
				headers: { Authorization: `Bearer ${created.adminToken}`, 'Content-Type': 'application/json' },
		...
		const blockedJoin = await router.fetch(
			request(routes.joinGame.href({ code: created.code }), {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
		...
		const rejoin = await router.fetch(
			request(routes.rejoinGame.href({ code: created.code }), {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
		...
		const clone = await router.fetch(
			request(routes.cloneGame.href({ code: created.code }), {
				method: 'POST',
		...
		const end = await router.fetch(
			request(routes.endGame.href({ code: created.code }), {
				method: 'POST',
				headers: { Authorization: `Bearer ${created.adminToken}` },
			}),
		)
		...
		const chatAfterEnd = await router.fetch(
			request(routes.sendChatMessage.href({ code: created.code }), {
				method: 'POST',
				headers: { Authorization: `Bearer ${joined.playerToken}`, 'Content-Type': 'application/json' },
